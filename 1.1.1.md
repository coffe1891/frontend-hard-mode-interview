...
在创建Symbol时，可以为调试目的向其添加`description` \(描述\)。有时候能够直接访问代码中的`description` ，对调试是很有用的。ES10中为`Symbol`对象添加了只读属性 `description` ，该对象返回包含`Symbol`描述的字符串。
