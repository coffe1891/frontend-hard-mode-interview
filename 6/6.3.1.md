...
**函数处理的是对象，而函子则处理的是范畴。**所谓“范畴”，从编程的角度来理解，我们可以理解成高阶对象。那么，**函子是范畴到范畴的映射关系**。
...
{% hint style="info" %}
因为JavaScript不是强类型语言，为方便举例说明，本文会临时借用Java语言。
{% endhint %}
...
这个实现了map方法的[typeclass](http://learnyouahaskell.com/types-and-typeclasses#typeclasses-101)，我们就称之为函子。

所谓的map方法，就是将高阶对象这个容器打开，取出里面的简单对象，然后再对简单对象应用map方法传递过来的函数，再把应用函数后的结果又包装成高阶对象，最后将这个高阶对象返回。

这么绕一圈儿弄出个函子的概念有什么用呢？函子并不刻意造出来的，是先有了用法然后总结归纳出来的基础概念。有了函子，可以处理高阶对象，而且想要什么结果，传入对应的函数就行，传入不同的函数，产生不同的结果，比较灵活；而且代码也十分简洁。在后续文章，会发现函数式编程的代码一向表现得更简洁一些。
...
首先，最直观的角度来说，函数式风格的代码可以写得更精简，大大减少了程序员的键盘敲击输入量。
