...
//Ramda
{
    let sum = R.reduce((a,b)=>a+b.a,0);//data参数省略掉了
    let getList = R.filter((o) => o.a > 1);//data参数省略掉了
    let total = R.compose(sum,getList)(list);
    console.log(total);//>> 5
}
```

可以发现正是因为Ramda把数据放在参数的最后一个位置，同时每个函数都柯里化过，所以代码才变得更简洁，更易读。而且，需要传递的参数`data`不见了！！这就引出了一种无参数的编程风格：**Pointfree**，放在[下一篇](6.3.3.md)单独介绍。
