...
const user1 = new CoffeUser('abc', 'xxx', 3);
const user2 = new CoffeUser('def', 'yyy', 0);

// 这就是把属性定义在原型链上的致命缺点，你可以直接访问，但无法直接修改
console.log(user1.username); // Unknown
// 如果通过__proto__来访问原型的属性并修改，则有副作用
user1.__proto__.username = 'U';
console.log(user1.username); // 'U'
// WOW，无情地影响了另一个实例!!!
console.log(user2.username); // 'U'
```

缺陷：

* 无法方便地修改子类实例属性，如果强行通过子类实例的原型修改属性，则会连带影响到其他实例。
* 
## 02.构造函数式继承
...
console.log(user1.username); // coffe1891
console.log(user1.password); // 123456
console.log(user1.login()); // TypeError: juejinUser1.login is not a function
```
