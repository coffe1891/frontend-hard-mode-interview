...
上面这段代码，显示了小明**从依赖具体**的女孩小玉儿，**转为依赖抽象**的大学女生。这样之后，小明内心不用更换了女朋友的位置，因此没有心理包袱，感觉很轻松。然后第107行在构造函数里传递（注入）了具体的大学女生，这就叫做**依赖\(于\)注入**\(DI\)；并且把控制选择女友的权力让渡给了老天**\(**由`showMeCollegeGirl`函数来分配女孩\)，这叫做**控制反转**\(IOC\)。
