...
已经有点柯里化了，但是还不符合要求。因为这个代码柯里化后得到的新函数`curry`只支持传“1段”参数，如果连续多段地传参数比如`curry(a)(b)`这样的话就不支持了。

{% hint style="info" %}
笔者注：函数右边带的一个可以处理参数的小括号对\(\)，本书称为**“1段”** 。函数每能多带1段，我们称函数有多1段的能力。
{% endhint %}
...
function curry(fn, length) {
    ...
    return function (...args2) {
        ...
        if (args2.length < length) {
            ...
            //就可以多出“1段”以便可以处理掉剩余的参数，直到把所有应传给fn的参数都处理完。
