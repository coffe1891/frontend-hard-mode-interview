...
已经有点柯里化了，但是还不符合要求。因为这个代码柯里化后得到的新函数`curry`只支持传1个参数，如果连续、分段地传多个参数比如`curry(a)(b)`这样的话就不支持了。
...
看代码报错部分，是因为处理完第1个参数之后没有返回函数，怎么办呢？想办法返回函数就行了呗。一般这种情况可以用递归再封装一层。我们可以把这个primaryCurrying函数用作辅助函数（因为它已经能处理传1个参数的情况了），帮助我们写真正的 柯里化函数。
...
//设计真正的柯里化函数,其中的参数length用来记录fn应该处理的参数长度，也即形参的长度
function curry(fn, length) {
    length = length || fn.length/*fn的形参的长度*/;
    ...
    return function (...args2) {
        ...
        if (args2.length < length) {
            ...
            //递归，进一步柯里化
            return curry(primaryCurrying.apply(this, combined), length - args2.length);
        } 
...
这边其实是在初步的基础上，加上了递归的调用，只要已经处理的参数个数小于最初的fn.length，就会继续执行递归。来测试一下效果怎么样？
