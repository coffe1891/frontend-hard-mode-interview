...
已经有点柯里化了，但是还不符合要求。因为这个代码柯里化后得到的新函数`curry`只支持传1个参数，如果连续、分段地传多个参数比如`curry(a)(b)`这样的话就不支持了。代码如下：
...
看上面代码第7行报错部分，是因为处理完第1个参数之后没有返回函数，怎么办呢？想办法返回函数就行了呗。一般这种情况可以用递归再封装一层。我们可以把这个`primaryCurrying`函数用作辅助函数（因为它已经能处理1个参数了），帮助我们写真正的 柯里化函数。
...
function curry(fn, length) {
    ...
    return function (...args2) {
        ...
        if (args2.length < length) {
            ...
            //递归，进一步柯里化。这里调用了primaryCurrying函数，每调用一次primaryCurrying，
            //会处理完一个参数。也即递归一次，就可以处理掉一个参数，直到把所有应传给fn的参数都处理完。
