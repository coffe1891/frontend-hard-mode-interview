...
> 维基百科上说道：柯里化（Currying）是一种技术，它把接受多个参数的函数变成接受单一参数（最初函数的第一个参数）的函数，并且该函数返回一个新函数，这个新函数接受余下的参数……如此循环，直到最后返回结果。
...
柯里化的定义很明确，**必须满足的特性之一，就是把多参数函数的第一个参数，要放到一个新函数里面单独传参**。网上有很多文章举的例子很牵强，不适合用来说明柯里化的特性，本文的举例，都符合这一定义。

那么，柯里化额外的封装了一层，在javascript的世界里有什么具体的好处呢？
...
add(1)(2)(3);                //>> 6   柯里化
add(1, 2, 3)(4);             //>> 10  严格一点来讲不是柯里化
add(1)(2)(3)(4)(5);          //>> 15  柯里化
add(2, 6)(1);                //>> 9   严格一点来讲不是柯里化
```
