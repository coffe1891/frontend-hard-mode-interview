...
还记得刚才的类数组么？如果它想使用 Array 原型链上的slice方法，可以这样：
...
因为传递过去的this.handleMessage是一个函数内存地址，没有附带上下文对象，也就是说该函数没有绑定它的this指向。
...
2. 通过为`thisArg`的一个临时方法，这样`thisArg`就是调用该临时方法的对象了，会将该临时方法的this隐式指向到`thisArg`上（参考上一篇文章关于this指向的阐述）；
3. 执行`thisArg`的临时方法，并传递参数；
4. 删除临时方法，返回方法的执行结果。
...
Function.prototype.myCall = function(thisArg, ...arr) {
  ...
  //若调用myCall的函数，我们记作func；
  //根据上篇文章介绍，myCall的this指向func
  thisArg[specialMethod] = this; //给thisArg对象建一个临时的方法来储存

  //根据上篇文章介绍，func作为thisArg对象的一个方法被调用，那么func中的this
  //就指向thisArg对象。由此巧妙地完成将this指向到thisArg
...
认真读一下注释，基本就能理解了，主要还是用到上一篇文章判断`this`指向的规则。
...
经过测试，以下三种为false的情况，thisArg都会意外地绑定到window上：
