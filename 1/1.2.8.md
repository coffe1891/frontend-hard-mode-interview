...
第一部分：编译并执行 JavaScript 代码，完成内存分配、垃圾回收等；  
第二部分：为 JavaScript 提供一些对象或机制，使它能够与外界交互。

**这里的第一部分，是 Engine（JS引擎）；第二部分，是 Runtime（运行时）。**
...
Event Loop（事件循环） 是让 JavaScript 做到既是单线程，又绝对不会阻塞的核心机制，也是 JavaScript 并发模型（Concurrency Model）的基础，是用来协调各种事件、用户交互、脚本执行、UI 渲染、网络请求等的一种机制。简单来说，Event Loop 只不过是**实现异步回调的一种机制**而已。
...
![&#x5185;&#x5B58;&#x6A21;&#x578B;](../.gitbook/assets/image%20%286%29.png)
