...
JavaScript起源于1995年Netscape为自家浏览器创造的脚本语言LiveScript。后来因为Sun公司的Java语言兴起，Netscape为了沾热度将LiveScript改名为JavaScript，但其实Java和JavaScript没有一点关系。
...
EI于2015年6月发布了ES 6，也就是我们今天广泛使用的版本。这之后每年6月发布一个极小更新的ES版本，比如2016年6月发布的ES 2016叫做ES 7，而这个版本只增加了两个新特性。截止本书编写时，ES 11的提案已经定稿了。

综上可见，**JavaScript是ECMAScript规范的一种实现**。为方便读者理解交流，而且由于ECMAScript 6之后变化并不是特别大，如果没有特别说明，本书后面讲述的JavaScript对应的规范默认为 ES 6。
...
| 新特性 | 中文说明 |
| :--- | :--- |
| `Array.prototype.includes` | 数组包含方法 |
...
这个是与`Math.pow`有关的特性，还记得`i++`,`x += x`这种写法吗，指数运算符与之类似。与Python语言一样，JavaScript也采用两个星符号`**`来表示`Math.pow`。好处有两个：  
...
| 新特性 | 中文说明 |
| :--- | :--- |
| `Object.values` / `Object.entries` | 对象值和方法 |
| String padding | 字符串填充 |
| `Object.getOwnPropertyDescriptors` | 获取对象属性描述符 |
| Trailing commas in function parameter lists and calls | 函数参数列表和调用中允许尾随逗号 |
