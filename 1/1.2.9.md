...
在前端日常开发中，数组被使用得非常频繁。只有对数组各种方法充分掌握后，能有效提高工作效率。
...
由于排序取决于具体实现，因此无法保证排序的时间和空间复杂度。更多排序算法，可参考「贰.1.1 十大经典排序算法」。
...
在有些情况下，我们需要通过username来查询详细people详情，通常为了方便查询，我们需要将array转换成object。那么，通过使用reduce\(\)方法，我们可以使用一下这种方法：
