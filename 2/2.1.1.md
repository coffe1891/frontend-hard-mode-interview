...
((arr) => {
    ...
    for (let i = 0; i < length; i++) {
        //将树根结点与末位叶结点调换，此时数组arr第(length-1)~第(N-1)项都是已排序好的
        swap(0, length-1);
        //一次循环处理掉一个结点，所以接下来要处理的结点数-1
        length--;
        //下沉，重排树，让根节点是最小的
        sink(arr,0,length);
    }
    ...
    function buildMinHeap(arr) {
        let length = arr.length;
        //请注意，需要循环处理的下沉结点数是Math.floor(length/2)，
        //为什么是Math.floor(length/2)？读者可以画个草稿图归纳一下。
